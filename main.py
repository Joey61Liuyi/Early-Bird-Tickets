...
if __name__ == '__main__':
    ...
    def cifar100_lambda1(x):
        if args.filter == 'lowpass':
            return filters.gaussian_filter(x, args.sigma)
        else:
            return x

    def cifar100_lambda2(x):
        if args.filter == 'highpass':
            return my_gaussian_filter_2(x, 1/args.sigma, args.filter)
        else:
            return x
    def cifar100_lambda3(x):
        if args.sparsity_gt>0:
        ...
        train_loader = torch.utils.data.DataLoader(
            datasets.CIFAR100('./data.cifar100', train=True, download=True,
                           transform=transforms.Compose([
                               ...
                               transforms.RandomCrop(32),
                               transforms.RandomHorizontalFlip(),
                               transforms.Lambda(cifar100_lambda1),
                               transforms.Lambda(cifar100_lambda2),
                               transforms.ToTensor(),
                               transforms.Lambda(cifar100_lambda3),
                               transforms.Normalize((0.4914, 0.4822, 0.4465), (0.2023, 0.1994, 0.2010))
                           ])),
            batch_size=args.batch_size, shuffle=True, **kwargs)
        test_loader = torch.utils.data.DataLoader(
